...
export default function typeCheck(program: Program, functions: StackFunctions, newFunctions: {name: string, type: StackType}[]): TSError[] {
    const programTypeErrors = traverseCheckProgram(program, {}, StackType.Int, functions);

    let functionErrors = [];
    for(const fn of newFunctions) {
        const functionError = traverseCheckProgram(functions[fn.type][fn.name].body, functions[fn.type][fn.name].params, fn.type, functions);
...
function traverseCheckProgram(program: Program, otherIdentifiers: Record<string, StackType>, stack: StackType, functions: StackFunctions): TSError[] {
    let errors: TSError[] = [];

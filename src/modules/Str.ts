...
const str: StackFunctions = {
  [StackType.Int]: {
    ...
    slice: {
      params: { string: StackType.Str, endIndex: StackType.Int, startIndex: StackType.Int },
      rawCode: (stacks, params) => {
        stacks[StackType.Str].push(params.string.slice(params.startIndex, params.endIndex));
      },
    },
  },
  [StackType.Float]: {},
  [StackType.Str]: {
    ...
    occurrence: {
        params: { search: StackType.Str,  string: StackType.Str },
        rawCode: (stacks, params) => {
          stacks[StackType.Int].push(params.string.split(params.search).length -1);
        },
      },
      toUpper: {
      ...
      replace: {
        params: { replacement: StackType.Str, search: StackType.Str,  string: StackType.Str },
        rawCode: (stacks, params) => {
          stacks[StackType.Str].push(params.string.replace(params.search, params.replacement));
        },
      },
      replaceAll: {
        params: { replacement: StackType.Str, search: StackType.Str,  string: StackType.Str },
        rawCode: (stacks, params) => {
          stacks[StackType.Str].push(params.string.replaceAll(params.search, params.replacement));
        },
      },
      repeat: {
        params: { string: StackType.Str, amount: StackType.Int },
      ...
      split: {
        ...
        rawCode: (stacks, params) => {
          params.string.split(params.separator).forEach(e => stacks[StackType.Str].push(e))
        },
      },
  },
  [StackType.Bool]: {},

...
export class TSError {
    ...
    constructor(pos: {startPos: Pos, endPos: Pos,}, msg: string, ...params: any[]) {
        ...
        if(!(errorLine.length < MAX_ERROR_WIDTH*2 + 1)) {
            ...
            if(textBefore.length >= MAX_ERROR_WIDTH) {
                ...
                newErrorLine = consoleEffect("... ", ConsoleEffects.Bright, ConsoleEffects.FgBlue) + textBefore.slice(-MAX_ERROR_WIDTH) + newErrorLine;
            }
            if(textAfter.length >= MAX_ERROR_WIDTH) {

                // need to trim off the end
                newErrorLine = newErrorLine + textAfter.slice(0, MAX_ERROR_WIDTH) + consoleEffect(" ...", ConsoleEffects.Bright, ConsoleEffects.FgBlue);
            }
